...
import { Sparkles, Send, TrendingUp, TrendingDown, Minus, X } from 'lucide-react';
...
export function InsightBot() {
  ...
  return (
    <>
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-24 right-4 z-40 flex h-[32rem] max-h-[calc(100dvh-7rem)] w-[calc(100vw-2rem)] max-w-sm animate-fade-in flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl dark:border-white/[0.08] dark:bg-surface-900 sm:right-6">
          {/* Header */}
          <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3.5 dark:border-white/[0.06]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-xs text-slate-500 dark:text-slate-400">Sales, revenue, clients & more</p>
            </div>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="flex size-8 shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-white/[0.06] dark:hover:text-slate-200"
              aria-label="Close"
            >
              <X className="size-4" />
            </button>
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-5 right-4 z-40 flex size-14 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-violet-500 text-white shadow-lg shadow-brand-500/30 transition-transform hover:scale-105 active:scale-95 sm:right-6"
        aria-label={open ? 'Close AI Insights' : 'Open AI Insights'}
      >
        {open ? <X className="size-6" /> : <Sparkles className="size-6" strokeWidth={2} />}
      </button>
    </>
  );
}
